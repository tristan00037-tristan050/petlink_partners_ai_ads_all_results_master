...
# Bootpay REST 키는 샌드박스 네트워크 연동 시만 주입(없으면 오프라인 샌드박스)
export BOOTPAY_API_BASE="${BOOTPAY_API_BASE:-}"         # 예: https://api.bootpay.co.kr (실서버), 샌드박스는 문서 기준 URL
...
# ── 0) 선행 스키마 존재 확인 ──
psql "$DATABASE_URL" -Atc "select 1 from information_schema.tables where table_name='ad_invoices';" | grep -q 1 \
  || { echo "[ERR] ad_invoices 없음. 이전 단계(r5.x) 먼저 반영"; exit 1; }
psql "$DATABASE_URL" -Atc "select 1 from information_schema.tables where table_name='ad_payments';" | grep -q 1 \
  || { echo "[ERR] ad_payments 없음. 이전 단계(r5.x) 먼저 반영"; exit 1; }
psql "$DATABASE_URL" -Atc "select 1 from information_schema.tables where table_name='payment_methods';" | grep -q 1 \
  || echo "[INFO] payment_methods는 이후 보강 코드가 처리"
...
# ── 3) Bootpay REST 어댑터(오프라인/샌드박스 겸용, node-fetch 의존 제거) ──
...
# ── 4) ads_billing 라이브러리에 setMethod 보강(없으면 추가) ──
...
# ── 5) /ads/billing 라우트에 /charge + /admin/verify 통합(누락 보완) ──
# adapter import + db import 보강
...
# ── 7) 스모크: 프리플라이트 → CHARGE → VERIFY → 상태확인 ──
# 프리플라이트
...
# 기본 수단 없을 수 있으니 한 개 삽입(샌드박스는 없어도 진행 가능)
psql "$DATABASE_URL" -c "
  INSERT INTO payment_methods(advertiser_id,pm_type,provider,token,brand,last4,is_default) 
  VALUES (${ADV},'CARD','bootpay','tok-demo-$(date +%s)','VISA','4242',TRUE) 
  ON CONFLICT (advertiser_id,provider,token) DO NOTHING;
  UPDATE payment_methods SET is_default=TRUE WHERE advertiser_id=${ADV} AND id=(SELECT id FROM payment_methods WHERE advertiser_id=${ADV} ORDER BY id DESC LIMIT 1);
" >/dev/null 2>&1 || true

# CHARGE
echo "[SMOKE] CHARGE"
CHARGE_RESP=$(curl -s -XPOST "http://localhost:${PORT}/ads/billing/charge" \
  -H "Content-Type: application/json" \
  -d "{\"invoice_no\":\"${INV}\",\"advertiser_id\":${ADV},\"amount\":${AMT}}" 2>&1)
if echo "$CHARGE_RESP" | grep -q '"status":"CAPTURED"'; then
  echo "SANDBOX CHARGE OK"
else
  echo "[FAIL] CHARGE: $CHARGE_RESP"
fi

# VERIFY(관리자 보호)
echo "[SMOKE] VERIFY"
VERIFY_RESP=$(curl -s -XPOST "http://localhost:${PORT}/ads/billing/admin/verify/${INV}" -H "X-Admin-Key: ${ADMIN_KEY}" 2>&1)
if echo "$VERIFY_RESP" | grep -q '"status":"CAPTURED"'; then
  echo "VERIFY OK"
else
  echo "[FAIL] VERIFY: $VERIFY_RESP"
fi
