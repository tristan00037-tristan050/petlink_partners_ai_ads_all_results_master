...
echo "[4/8] 라우트: /ads/billing/* (결제수단/청구/확정/웹훅)"
...
if [ "${ENABLE_CONSUMER_BILLING}" = "false" ]; then
  sed -i.bak 's|^app.use("/billing",|// app.use("/billing",|' server/app.js 2>/dev/null || true
...
echo "[ads-billing overlay] ready"
